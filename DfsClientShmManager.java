 ...
 *
 * 一个DFSClient可能需要维护与多个Datanode之间的共享内存，
 * 所以DfsClientShmManager还定义了EndpointShmManager来管理与一个Datanode之间的所有共享内存
...
public class DfsClientShmManager implements Closeable {
  ...
  class EndpointShmManager {
    ...
    Slot allocSlot(DomainPeer peer, MutableBoolean usedPeer,
      ...
      while (true) {
        if (closed) {
          // 如果DfsClientShmManager已经关闭了，返回null
          if (LOG.isTraceEnabled()) {
        ...
        if (loading) {
          ...
          try {
            ...
            // 将已经申请的共享内存加入domainSocketWatcher，监控状态
    ...
    void freeSlot(Slot slot) {
      ...
      if (shm.isDisconnected()) {
        ...
        if (shm.isEmpty()) {
          ...
          // 如果共享内存已经被标记为过期，并且共享内存中的所有槽位都已经被释放了，则将共享内存释放
          shm.free();
        }
      } else {
        ShmId shmId = shm.getShmId();
        ...
        // 如果共享内存中的所有槽位都空闲，则调用shutDown()方法将共享内存相关的domainSocket关闭
