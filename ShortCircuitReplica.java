 ...
 *
 * ShortCircuitReplica类封装了一个短路读数据块副本的所有信息，
 * 只有获取了ShortCircuitReplica对象，才能构造BlockReaderLocal对象完成短路读操作
 */
...
public class ShortCircuitReplica {
   ...
   *
   * Protected by the cache lock.
   * 当前ShortCircuitReplica对象是否在cache中被删除
   */
   ...
   *
   * Protected by the cache lock.
   * 标识当前ShortCircuitReplica对象被引用的次数
   */
  int refCount = 2;
   ...
   *
   * Protected by the cache lock.
   * ShortCircuitReplica对象被加入ShortCircuitCache的evictable队列的时间，如果不在evictable队列，则为null
   */
  ...
  /**
   * 将ShortCircuitReplica对应的数据块文件映射到内存中
   * 该方法是在客户端对副本进行零拷贝读取时调用的
   */
  MappedByteBuffer loadMmapInternal() {
    try {
      FileChannel channel = dataStream.getChannel();
      // 调用FileChannel.map()方法将数据块文件映射到内存中
