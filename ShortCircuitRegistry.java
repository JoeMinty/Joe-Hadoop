 ...
 *
 * ShortCircuitRegistry类用于管理Datanode侧的所有共享内存，包括创建、释放共享内存等操作。
 */
public class ShortCircuitRegistry {
  ...
  public static class RegisteredShm extends ShortCircuitShm
    ...
    public boolean handle(DomainSocket sock) {
      synchronized (registry) {
        synchronized (this) {
          // 调用ShortCircuitRegistry.removeShm()方法处理domainSocket关闭的情况 
  ...
  */
   ...
   * 当Datanode将数据块从缓存中移除时，会调用这个方法删除Slot对象的可锚状态位
